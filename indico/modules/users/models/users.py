...
class ProfilePictureSource(int, Enum):
    ...
    @property
    def is_gravatar(self):
        """Whether the picture source is gravatar-based.

        This is the case for actual gravatars but also for identicons, since these
        are generated by gravatar as well.
        """
        return self in {ProfilePictureSource.identicon, ProfilePictureSource.gravatar}
...
class User(PersonMixin, db.Model):
    ...
    def _synchronize_email(self, email, silent=False):
        ...
        if not config.DISABLE_GRAVATAR and self.picture_source.is_gravatar:
